...
function updateTime() {
    ...
    for (const item of time_update_list) for (const element of document.querySelectorAll(item.selector)) element.innerHTML = d.slice(item.start, item.end)
}
...
function addStorageField(id, selector, name, placeholder, filter = x => x, callback = () => {
    ...
    for (const element of elements) element.innerHTML = filter(init_value), element.addEventListener("click", () => {
        ...
        for (const _item of fields[id]) for (const _element of document.querySelectorAll(_item.selector)) _element.innerHTML = _item.filter(res || _item.placeholder)
    });
    callback(init_value)
}
