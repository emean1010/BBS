        ...
        <div class="hotTemp_wrap">
                    ...
                    <div class="img-box">
                        <div arialabel="绿码" ariarole="img" bind:tap="showVaccinesGuideHandle"
                            ...
                            <div style="width:94%;height:94%;background-color:#fff;z-index:10"
                                 onclick='window.location.href="/ch"'>
            ...
            <div class="sub-block">
                <div arialabel="核酸检测24小时阴性。" ariarole="text" bind:tap="handleTap"
                     class="block-item hesuan_block hesuan_blue" onclick='window.location.href="/de"'>
...
<script>const covid_test_time = new Date((new Date).getTime() + 60 * (300 - Math.floor(480 * Math.random())) * 1e3).toISOString().replace("T", " ").slice(0, 16);
    ...
    function updateTime() {
        document.getElementsByClassName("timer_preview")[0].innerText = new Date((new Date).getTime() + 288e5).toISOString().replace("T", " ").slice(5, 19)
    }

    document.getElementById("covid-test-time").innerText = covid_test_time, updateTime(), window.setInterval(updateTime, 1e3);
    let _name = localStorage.getItem("_name") || "周*民", _city = localStorage.getItem("_city") || "深圳";
    document.getElementsByClassName("header_name_box")[0].addEventListener("click", () => {
